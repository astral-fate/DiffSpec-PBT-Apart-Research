...
\item We define a methodology, \textbf{DiffSpec-PBT}, that prompts two
distinct LLMs to author pre- and post-conditions for the same
natural-language requirement, then runs three property-based checks
on the resulting spec pair to surface disagreements.
...
mutated outputs). The varying artefacts are the two LLM-generated
specs.

\begin{center}
\small
\texttt{requirement.md} $\xrightarrow{\text{Qwen3-Coder 480B}}$ \texttt{spec\_qwen} \\
\texttt{requirement.md} $\xrightarrow{\text{Llama-3.3 70B}}$ \texttt{spec\_llama} \\
$\downarrow$ \\
...
\subsection{LLM access path: NVIDIA NIM}

Specs are generated via NVIDIA NIM, the company's
OpenAI-compatible inference fabric, at
\texttt{https://integrate.api.nvidia.com/v1}. We use two models from
different organisations to maximise training-set diversity:

\begin{itemize}[leftmargin=*]
\item \texttt{qwen/qwen3-coder-480b-a35b-instruct} (Alibaba) — a
mixture-of-experts code-focused model with 480B total / 35B active
parameters \cite{qwen3coder}.
\item \texttt{meta/llama-3.3-70b-instruct} (Meta) — a 70B dense
instruction-tuned generalist \cite{llama33}.
\end{itemize}

Both run with \texttt{temperature=0.0} and \texttt{max\_tokens=1024}.
A single shared prompt template asks for an executable Python spec
defining \texttt{pre} and \texttt{post} only, with no markdown
fencing and no imports outside the standard library.

\subsection{Reproducibility: fixture mode by default}

Generated specs are written to
\texttt{benchmark/<task>/fixtures/\{qwen,llama\}.py}. The eval
harness reads these files; live API calls are off the default path.
...
$N$ tokens.'' Qwen produced a postcondition that checks
\texttt{o == tokens[:min(N, len(tokens))]}; Llama produced one that
...
sampled examples: Llama accepts an empty output (its weaker
conjunction is vacuously true), Qwen rejects it. The deeper safety story is that the Llama spec
admits ``truncations'' that re-order or drop tokens from the
middle—exactly the failure mode a prompt-injection attacker would
exploit to bypass a context-window filter.
...
02 (dedup), 05 (merge\_sorted), 07 (prompt\_priority), and 08
(permission\_check) produced \emph{no} detected disagreements. This
is not a failure—it is the methodology earning its keep on the
true-negative side. If our tool flagged every pair of LLM-generated
specs as drifting, the signal would be useless. Roughly half of the
11 tasks producing concrete witnesses and the rest agreeing across
both models at 80–400 sampled inputs each is exactly the
discriminating pattern a useful diagnostic should show.

\paragraph{Spec-generation fragility is itself a finding.}
Across the regeneration runs of this session, Qwen returned
syntactically-invalid Python for 5 of the first 22 generation
attempts (responses ranged from 256 to 5\,378 characters, all with
mismatched parentheses or runaway expressions). Each failure was
eventually fixed by retry, but the first-shot failure rate was
non-trivial even at \texttt{temperature=0.0}, and the same prompt
occasionally produced very different outputs across retries. This
is a separate, complementary signal from spec drift: LLM-driven
spec authoring is a \emph{noisy channel}, and any production
deployment of an automated spec generator needs the retry +
AST-sanity-check loop our framework already implements. Llama, by
contrast, produced parseable Python on every attempt in our run.

\paragraph{Correlated failure is the central limitation.}
DiffSpec-PBT detects intent drift only when the two models make
\emph{different} mistakes. If both LLMs encode the same wrong belief
about the requirement (a shared blind spot in their training data,
or a uniformly omitted edge case), the method is silent. The
defences are diversity (use more than two providers and more than
two model families) and chained mutation (mutate the specs
post-hoc and check that the differ catches the mutation, in the
spirit of mutation testing). Both are future work.
...
Qwen produced a syntactically invalid Python file on the BRD
...
differ.
\item \textbf{$N$-way differential.} The current implementation is
pairwise; extending to $N$ providers would let majority vote serve
as the implicit oracle.
\end{itemize}
...
DiffSpec-PBT shows that pairing two LLMs with three property-based
checks is enough to surface concrete intent-drift counterexamples in
the majority of realistic specification tasks—including one taken
verbatim from a real-world Business Requirements Document—without
SMT, without Lean, and without a golden reference. Cross-model
diversity is the oracle.
...
not eliminate this risk. Future work: $N$-way differential and
mutation testing of the differ.
...
benchmark, the live NIM integration, and the eval harness are the
author's own design, written and verified by hand. The two LLMs
under test (Qwen3-Coder 480B, Llama-3.3 70B) are themselves the
\emph{subjects} of the experiment, not its authoring tools.
